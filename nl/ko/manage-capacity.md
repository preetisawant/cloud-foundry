...
관리자와 개발자는 애플리케이션 및 셀에서 사용되는 CFEE의 리소스 용량(메모리 및 CPU)을 확인할 수 있습니다. CFEE 인스턴스에서 리소스 사용량을 모니터하려면 다음을 수행하십시오.

1. [{{site.data.keyword.Bluemix_notm}} Cloud Foundry 대시보드](https://cloud.ibm.com/dashboard/cloudfoundry?filter=cf_environments)로 이동하여 리소스 사용량을 관리하려는 {{site.data.keyword.cfee_full_notm}}를 여십시오.
2. {{site.data.keyword.cfee_full_notm}} 사용자 인터페이스에서 왼쪽 탐색 분할창에 있는 **리소스 사용량** 항목으로 이동하여 _리소스 사용량_ 페이지를 여십시오. _리소스 사용량_ 페이지에서 _애플리케이션_ 또는 _셀_ 하위 항목으로 이동하여 해당 페이지를 열 수 있습니다.  _애플리케이션_ 및 _셀_ 페이지에 표시된 정보는 리소스 사용량을 분류하는 두 가지 방법으로 간주될 수 있습니다.
   ...
   * **셀** 페이지에는 특정 셀에서 실행 중인 애플리케이션 인스턴스의 리소스 사용량이 표시됩니다. 셀 간의 리소스 사용량 패턴은 용량 및 로드 분산에 대한 인사이트를 제공할 수 있습니다.  예를 들어, 애플리케이션의 로드 밸런싱(예: 셀 간에 애플리케이션에서 사용된 사용량에 큰 차이가 있는 경우) 또는 전체 용량에 근접한 리소스 사용량(예: 모든 셀의 리소스 사용량 백분율이 큰 경우)에 대한 문제점을 식별하는 데 도움이 될 수 있습니다.
...
1. 실제 및 예약 메모리 사용량을 표시하는 가로 막대형 차트.
   ...
   **참고:** 애플리케이션이 실제 사용 가능한 양을 초과하는 메모리를 예약하는 경우, 선택된 애플리케이션에 의해 과도하게 예약된 메모리 양을 나타내기 위해 예약 메모리 막대형 차트에서 빨간색 점의 윤곽선이 표시됩니다.
   ...
   모든 애플리케이션 또는 테이블에서 선택한 애플리케이션에서 사용된 메모리 백분율을 나타내는 수를 표시하려면 차트의 해당 부분 위로 마우스를 이동하십시오.  차트의 *모든 앱* 부분 위로 마우스를 이동하면 총 사용 가능 메모리와 비교하여 CFEE의 모든 애플리케이션에서 사용된 메모리 백분율이 표시됩니다. 차트의 *선택한 앱* 부분 위로 마우스를 이동하면 총 사용 가능 메모리와 비교하여 테이블에서 선택한 애플리케이션에서 사용된 메모리 백분율이 표시됩니다.
...
2. 모든 셀과 셀에서 실행 중인 애플리케이션 인스턴스를 나열하는 테이블.  테이블의 각 행에는 해당 셀 및 애플리케이션 인스턴스에 대한 리소스 정보가 표시됩니다.
   ...
   * **CPU(셀의 %)**: CFEE에서 사용 가능한 총 CPU 중에서 셀에서 실행 중인 애플리케이션 인스턴스에서 사용된 CPU의 백분율입니다.
...
CFEE에서 사용 가능한 메모리 메트릭의 유형은 다양합니다.  다양한 관점으로 메모리 이용률을 분석할 수 있으므로 이와 같이 다양한 메모리 메트릭을 사용할 수 있습니닫. 메모리 이용률은 Cloud Foundry 셀에 사용 가능한 용량과 비교하거나 셀을 프로비저닝할 Kubernetes 작업자 노드의 실제 총 용량(Cloud Foundry와 관련되지 않은 기타 메모리 이용률 포함 가능)과 비교하여 측정할 수 있습니다. 메모리 용량은 애플리케이션에서 예약(할당)한 메모리 관점 또는 해당 애플리케이션에서 실제로 사용하는 메모리 관점에서도 볼 수 있습니다.  

다음은 CFEE 인스턴스의 사용자가 사용할 수 있는 메모리 메트릭입니다.

* Cloud Foundry 셀을 지원하는 Kubernetes 작업자 노드의 총 메모리 용량과 비교하여 해당 노드에서 사용한 전체 메모리.  이 메트릭은 Cloud Foundry 셀뿐 아니라 Cloud Foundry와 관련되지 않은 시스템 또는 캐시 오버헤드에서도 사용하는 작업자 노드 메모리를 나타냅니다.  이 메트릭은 **개요** 페이지의 **전체 사용량** 게이지에 표시됩니다.
* 메모리의 실제 사용 여부에 상관없이 사용 가능한 셀 메모리와 비교하여 애플리케이션에 할당된 메모리. 이 메트릭은 **개요** 페이지의 **할당됨** 게이지에 표시됩니다.
* 사용 가능 셀 메모리와 비교하여 애플리케이션에서 실제로 사용한 메모리. 사용된 전체 애플리케이션 메모리는 **개요** 페이지의 **앱 사용량** 게이지에 표시됩니다. 특정 애플리케이션에서 사용한 메모리는 **리소스 사용량 - 애플리케이션** 페이지에 표시됩니다.
* 작업자 노드의 메모리 용량과 비교하여 Cloud Foundry 셀에서 사용한 메모리.  이 메트릭은 **리소스 사용량 - 셀** 페이지에 표시됩니다.
* 작업자 노드에서 사용한 총 메모리(Cloud Foundry와 비교하거나 비교하지 않음). 이 메트릭은 CFEE 셀을 지원하는 노드와 CFEE 제어 플레인을 지원하는 노드 둘 다의 **업데이트 및 스케일링** 페이지에 표시됩니다.
* 추가 메모리 메트릭은 Grafana 대시보드에 표시됩니다. 이 대시보드는 **모니터링** 페이지에서 시작할 수 있습니다.  이 메트릭에서는 Cloud Foundry 셀과 Kubernetes 클러스터 및 작업자 노드의 자세한 메모리 용량과 사용량을 보여줍니다.
