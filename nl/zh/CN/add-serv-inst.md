...
{{site.data.keyword.Bluemix}} 帐户中可用的公共服务实例本身不会自动提供给 CFEE 环境。为了使公共服务实例（在 {{site.data.keyword.Bluemix}} 帐户中可用）变为可供 CFEE 环境中的空间使用，必须将其明确添加到目标 CFEE 空间。当您通过 CFEE 的用户界面将公共服务实例_添加_到 CFEE 时，将在 CFEE 中放置公共服务实例的别名。将 {{site.data.keyword.Bluemix}} 服务实例添加（创建别名）到 CFEE 空间后，即可以将其绑定到该 CFEE 空间中的应用程序。这允许开发者在 CFEE 环境中部署的应用程序中利用大量 {{site.data.keyword.Bluemix}} 服务，同时允许对这些 {{site.data.keyword.Bluemix}} 服务进行访问控制。
...
您还可以从 CFEE 空间创建服务实例，要么从 CFEE 的用户界面，要么使用 CLI（请参阅下面的各部分）。使用此命令从 CFEE 创建服务实例会产生双重结果：
* 在公共 {{site.data.keyword.Bluemix}} 中创建服务实例。
* 在创建服务实例的 CFEE 空间中创建该公共服务实例的别名。
    
以下各部分描述了如何在用户界面中以及如何使用 Cloud Foundry CLI 来对服务实例执行添加、创建、控制访问和绑定操作。
   

## 查看所有 CFEE 环境中的 {{site.data.keyword.Bluemix_notm}} 服务实例
{: #viewing-services_across}

转至 [Cloud Foundry 服务仪表板](https://cloud.ibm.com/dashboard/cloudfoundry/services)，以查看 {{site.data.keyword.Bluemix_notm}} 帐户中您有权访问的所有 {{site.data.keyword.Bluemix_notm}} 服务实例的统一视图，并查看哪些实例已添加到哪些 CFEE 空间。
...
* 要添加其实例的服务不能是 Cloud Foundry 服务。只有支持资源组和 IAM 的 {{site.data.keyword.Bluemix_notm}} 服务可以添加（创建别名）到 CFEE。实例化到公共 Cloud Foundry 组织、空间和角色的服务无法添加（创建别名）到 CFEE。{{site.data.keyword.Bluemix_notm}} 服务正逐步转为使用资源组而受益。服务转为使用资源组而非 Cloud Foundry 组织和空间后，您可以将该服务先前存在的任何实例迁移到资源组，迁移时可将其添加到 CFEE。请参阅[将 Cloud Foundry 服务实例和应用程序迁移到资源组](https://cloud.ibm.com/docs/resources/instance_migration.html#migrate)。
...
1. 转至 [Cloud Foundry 服务仪表板](https://cloud.ibm.com/dashboard/cloudfoundry/services)。  
...
1. 在 {{site.data.keyword.Bluemix_notm}} [仪表板](https://cloud.ibm.com/dashboard)或 [Cloud Foundry 服务仪表板](https://cloud.ibm.com/dashboard/cloudfoundry/services)中，找到托管应用程序的 Cloud Foundry Enterprise Environment。
...
* 在 IBM Cloud 中创建 {{site.data.keyword.Bluemix_notm}} 服务实例。这等同于在 {{site.data.keyword.Bluemix_notm}} [目录](https://cloud.ibm.com/catalog)中创建服务实例。
...
## 使用 Cloud Foundry CLI 在 CFEE 空间中创建 {{site.data.keyword.Bluemix_notm}} 服务实例
...
5. 创建服务：
您可以发出 `cf create-service` 命令以在发出命令的 CFEE 空间中创建服务实例。使用此命令从 CFEE 创建服务实例会产生双重结果：
    * 在公共 {{site.data.keyword.Bluemix}} 中，使用 `instance_name` 参数提供的名称创建服务实例。如果命令中未提供 `instance_name`，那么 {{site.data.keyword.Bluemix}} 资源控制器将提供名称。  
    * 在发出命令的 CFEE 空间中，使用由 `SERVICE_INSTANCE` 指定的名称创建该公共服务实例的别名。请注意，公共服务实例的名称不会自动与 CFEE 中的服务实例同名。因此，如果要为公共服务实例和 CFEE 中的服务实例（公共服务实例的别名）提供相同的名称，那么需要在以下命令中将 `instance_name` 和 `SERVICE_INSTANCE` 指定为相同的值：
...
以下示例会在“bluemix-us-south 区域”中创建名为“myCloudant”（公共实例与 CFEE 别名同名）的 Cloudant 服务实例（标准套餐），并将其分组到特定资源组中：
 ...
 可以使用可选参数发出 `create-service` 命令来处理特定用例：
 
   * 实例名称 (`instance_name`)：允许您为在公共 {{site.data.keyword.Bluemix}} 中创建的服务实例指定定制名称。如果未提供 `instance_name` 值，那么公共服务实例的缺省名称将与 `SERVICE_INSTANCE`（CFEE 中实例的名称）不同。要控制公共服务实例的名称，或者如果您要为其提供与 CFEE 中的服务实例相同的名称（公共服务实例的别名），我们建议您使用此参数。
   * 资源组 (`resource_group`)。允许您指定用于放置新实例的资源组。"resource_group" (`resource_group`)。请注意，此参数的值必须是资源组的标识 (_resource group ID_)，而不是资源组的名称 (_resource group name_)。您可以使用 `ibmcloud resource groups` 命令找到特定资源组的 _resource group ID_。
   * 目标部署区域 (`target`)。要供应服务实例的区域。请注意，某些服务不需要指定目标。但是，如果所要创建的服务需要目标而未提供任何目标，那么该命令将失败。

（可选）您可以提供包含这些参数的 JSON 文件。例如：
  ```
  {
    "NewCloudant": 
        {  
...
然后，可以将该 JSON 文件作为 `cf create-service` 命令的一部分进行调用。JSON 文件的路径可以是绝对或相对路径：
  ```
  cf create-service SERVICE PLAN SERVICE_INSTANCE -c PATH_TO_FILE
  ```
  {: pre}
   
您可以通过发出以下命令来获取更多详细信息：
  ```
...
## 从 CFEE 的用户界面将服务绑定到应用程序
{: #bind-services-ui}

对 {{site.data.keyword.Bluemix_notm}} 服务实例具有_操作员_平台角色（或更高级别角色）和_写入者_服务角色（或更高级别角色）的用户可以在 [Cloud Foundry 服务仪表板](https://cloud.ibm.com/dashboard/cloudfoundry/services)或 CFEE 空间用户界面的“服务”选项卡中，将该实例绑定到 CFEE 空间中部署的应用程序。   

要在 [Cloud Foundry 服务仪表板](https://cloud.ibm.com/dashboard/cloudfoundry/services)中将服务实例绑定到应用程序，请执行以下操作：
1. 转至 [Cloud Foundry 服务仪表板](https://cloud.ibm.com/dashboard/cloudfoundry/services)，以查看 {{site.data.keyword.Bluemix_notm}} 帐户中可用的所有 {{site.data.keyword.Bluemix_notm}} 服务实例的统一视图。该视图还旨在显示哪些 {{site.data.keyword.Bluemix_notm}} 服务实例在哪些 CFEE 空间中可用（创建了别名）。可以展开服务实例以显示添加了该服务实例的 CFEE 空间。您可以进一步展开这些空间，以查看这些 CFEE 空间中所添加服务实例已绑定到的应用程序。 
...
5. 在__绑定应用程序__对话框中，选择要绑定的应用程序。 
   ...
   **注：** 如果要绑定的服务同时支持公共（外部）和专用（内部）端点（请参阅 [IBM Cloud Service Endpoint](https://cloud.ibm.com/docs/services/service-endpoint?topic=service-endpoint-about#about)）和/或支持多个[服务访问角色](https://cloud.ibm.com/docs/iam?topic=iam-iamconcepts#am)（用于指定可通过绑定来执行的针对服务实例的允许操作），那么将显示多步骤对话框以提示您选择端点类型（公共或专用）和/或特定服务角色。
...
## 使用 Cloud Foundry CLI 将服务绑定到应用程序
{: #bind-services-cli}

可以使用 `cf bind-service` 命令将服务实例绑定到 CFEE 中的应用程序:
...
* 要使用 `cf bind-service` 命令将应用程序绑定到支持通过 IBM Cloud 专用网络连接到 IBM Cloud 服务的 [IBM Cloud Service Endpoint](https://cloud.ibm.com/docs/services/service-endpoint?topic=service-endpoint-about#about) 的服务实例。要将应用程序（部署在 CFEE 中）绑定到支持公共（外部）和专用（内部）端点的服务，您需要指定要将哪种端点用于绑定。在以下示例中，该命令使用“内部”端点绑定服务
...
* 当服务具有多个[服务访问角色](https://cloud.ibm.com/docs/iam?topic=iam-iamconcepts#am)时。服务访问角色指定可以通过绑定来执行的针对服务实例的允许操作。您可以通过 `role` 参数指定特定服务访问角色。在以下示例中，绑定指定了“writer”服务访问角色：
  ...
  {: pre}
<br>
有关使用 `cf bind-service` CLI 命令绑定应用程序的更多信息，请参阅 Cloud Foundry 文档中的[绑定服务实例](https://docs.cloudfoundry.org/devguide/services/application-binding.html){: new_window} ![外部链接图标](../icons/launch-glyph.svg "外部链接图标")。
...
按照上文所述的一般行为，我们建议通过发出以下命令来控制组织对服务的访问权：
  ...
  * **禁用**所有组织对服务套餐的访问权：
  ...
  以下示例禁止 MyOrg 的所有成员访问 Cloudant 服务的标准套餐：
  ```
  ...
  * **启用**特定 CFEE 组织对服务套餐的访问权。这将对在该命令中未明确启用的其他所有组织禁用该服务套餐。 
  ```
...
CFEE 空间中的开发者可以使用 IBM Cloud 帐户中可用的现有服务实例。在 CFEE 的特定空间内，用户可以向该空间**添加**服务实例（请参阅上面的[添加现有服务实例](https://cloud.ibm.com/docs/cloud-foundry/add-serv-inst.html#workingwith-services#adding-services-inspace)）。向 CFEE 空间添加服务实例会创建服务实例的别名（对服务实例的引用），这将允许开发者使用别名将服务实例绑定到部署在该 CFEE 空间中的应用程序，就像使用实际服务实例一样。

帐户管理员可以通过 [IAM 访问策略](https://cloud.ibm.com/docs/iam/iamusermanage.html#iamusermanage)来控制服务实例的使用。在 UI 或 ibmcloud CLI 中，帐户管理员可以分配针对服务实例本身的角色，或分配针对这些实例所在的资源组的角色。开发者需要针对服务实例（或其资源组）的_查看者_角色或更高角色，才能将该实例添加到空间。
