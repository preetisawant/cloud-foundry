...
1. 垂直長條圖，其中顯示：
   ...
   * 下表中的**系統** 使用的記憶體及 CPU。系統用量代表 CFEE 服務元件項目加上應用程式快取所使用的資源。
...
2. 此表格列出其中正在執行的所有 Cell 及應用程式實例。表格中的每一列都會顯示該 Cell 及應用程式實例的資源資訊。
   ...
   * **CPU (百分比)**：Cell 上執行之應用程式實例所使用 CFEE 中的總計可用 CPU 百分比。
