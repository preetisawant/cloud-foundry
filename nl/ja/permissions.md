...
* **{{site.data.keyword.cfee_full_notm}} サービス**のリソースに対する_管理者_ または _エディター_ の役割。環境が割り当てられているリソース・グループ内で、{{site.data.keyword.cfee_full_notm}} サービス内の管理者またはエディターの役割を持つユーザーは、環境を作成および削除できます。 ただし、{{site.data.keyword.cfee_full_notm}} インスタンスにユーザーを割り当てたり、そのインスタンス内のユーザーに割り当てられている役割を変更したりできるのは、管理者役割を持つユーザーのみになります。
...
2. アカウント管理者によって以下の IAM _アクセス・ポリシー_ が付与されていなければならない ({{site.data.keyword.Bluemix_notm}} ヘッダーの[**「管理」>「アカウント」>「ユーザー」**](https://console.bluemix.net/iam/#/users)メニューの下の_「ID およびアクセス」_ページを参照して、現行アカウントのアクセス・ポリシーを確認してください)。
    ...
    すべてのユーザーには、環境インスタンスが作成されたリソース・グループ内の {{site.data.keyword.cfee_full_notm}} インスタンスに対する_ビューアー_ 以上のアクセス権限が必要です。ユーザーが {{site.data.keyword.cfee_full_notm}} インスタンスで持つアクセス・レベルおよび制御レベルは、次のようにアクセス・ポリシーで付与された役割によって決まります。
  - _管理者_ または_エディター_ の役割を割り当てられたユーザーは、組織を作成し、組織およびスペースに管理者を割り当て、環境内のすべての組織およびスペースに対して完全な許可を持ち、Cloud Controller API を使用して操作アクションを実行できます。これらのユーザーには、Cloud Foundry の_「ユーザー・アカウントおよび認証スコープ」_で _cloud_controller.admin scope_ が自動的に付与されます。
  - CFEE に対して_ビューアー_ の役割を持つユーザーは、メインの {{site.data.keyword.Bluemix_notm}} ダッシュボードに CFEE が表示され、そのユーザー・インターフェースを開くことができます。環境内の特定の組織およびスペースへのユーザー・アクセス権限は、それらの組織およびスペースの管理者によって割り当てられる特定の組織およびスペースの役割で管理されます。 詳しくは、[組織へのユーザーの追加](add-users.html)を参照してください。

     **注:** CFEE インスタンスがグループ化されたリソース・グループに対する_ビューアー_ のアクセス権限だけでは、CFEE を表示するために十分ではありません。CFEE インスタンスが表示されるためには、そのインスタンスに対する明示的なアクセス権限 (ビューアーの役割以上) が必要です。

  - CFEE スペースにデプロイされたアプリケーションに {{site.data.keyword.Bluemix_notm}} サービスのインスタンスを**バインド**するには、そのサービスに対する_エディター_ 以上の役割が必要です。

  - **CFEE を新規バージョンに更新**するには、CFEE インスタンスに対する_エディター_ 以上の役割と、その CFEE がプロビジョンされた Kubernetes クラスターに対する_オペレーター_ 以上の役割が必要です。

  - CFEE の**容量を変更** (セルを追加または削除) するには、CFEE インスタンスに対する_管理者_ の役割と、その CFEE がプロビジョンされた Kubernetes クラスターに対する_オペレーター_ 以上の役割が必要です。
